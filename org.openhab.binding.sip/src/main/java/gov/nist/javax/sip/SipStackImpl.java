 ...
 * </li>
 *
 * <li><b>gov.nist.javax.sip.MESSAGE_PARSER_FACTORY = name of the class implementing
 * gov.nist.javax.sip.parser.MessageParserFactory</b>
 * This factory allows pluggable implementations of the MessageParser that will take care of parsing the incoming
 * messages.
 * By example one could plug a lazy parser through this factory.</li>
 *
 * <li><b>gov.nist.javax.sip.MESSAGE_PROCESSOR_FACTORY = name of the class implementing
 * gov.nist.javax.sip.parser.MessageProcessorFactory</b>
 ...
 * gov.nist.javax.sip.stack.timers.SipTimer</b> interface
 ...
 * on how and when it accesses transaction and dialog data since it cleans up aggressively when transactions changes
 * state
 ...
 * A property that specify the strategy for cleaning up references within the Dialog and Transaction structures.
 * Following options can be used
 * <ul>
 * <li>None: This is the default. It keeps references to request and responses as part of transactions and dialogs. This
 * consumes the most amount of memory usage</li>
 * <li>Normal: It removes references to request and responses in transactions and dialogs in cleaning them up but keep
 * byte arrays of requests or responses to be able to reparse them if the application need them. This gives a 2x
 * improvement in memory as opposed to None</li>
 * <li>Aggressive: It removes references to request and responses in transactions and dialogs in cleaning them up and
 * don't allow reparsing. Need careful application design. This gives a further improvements in memory as opposed to
 * Normal Strategy and also CPU improvements.</li>
 * </ul>
 * </li>
 ...
 * A property that will specify for how many seconds the Dialog and Transaction structures will stay in memory before
 * the stack releases them</li>
 *
 *
 ...
 * </li>
 *
 * <li><b>gov.nist.javax.sip.SIP_MESSAGE_VALVE= String</b> Default to null. The class name collection of your custom
 * valve components. The classes
 * are separated by comma and the order will be honored later when invoking the callbacks. All instances of these
 * classes will be created and
 * the SIPMessageValve.processRequest/Response() methods will be called for every message before any long-lived SIP
 * Stack resources are allocated
 * (no transactions, no dialogs). From within the processRequest callback implementation you can drop messages, send a
 * response statelessly or
 * otherwise transform/pre-process the message before it reaches the next steps of the pipeline. Similarly from
 * processResponse() you can manipulate
 * a response or drop it silently, but dropping responses is not recommended, because the transaction already exists
 * when the request for the response
 * was sent.
 * </li>
 *
 * <li><b>gov.nist.javax.sip.SIP_EVENT_INTERCEPTOR</b> Default to null. The class name of your custom interceptor
 * object.
 ...
 * analysis logic as SIP messages advance through the pipeline checkpoints. One example implementation of this
 * interceptor
 ...
 * occur only on certain events, so it will not overwhelm the CPU. The overall performance penalty by using this class
 * in
 * production under load is only 2% peak on average laptop machine. There is minimal locking inside. One known
 * limitation
 ...
 * </li>
 *
 * <li><b>gov.nist.javax.sip.TLS_CLIENT_AUTH_TYPE = String </b> Valid values are Default (backward compatible with
 * previous versions)
 * , Enabled, Want, Disabled or DisabledAll. Set to Enabled if you want the SSL stack to require a valid certificate
 * chain from the client before
 * accepting a connection. Set to Want if you want the SSL stack to request a client Certificate, but not fail if one
 * isn't presented.
 * A Disabled value will not require a certificate chain for the Server Connection. A DisabledAll will not require a
 * certificate chain for both Server and Client Connections.
 * </li>
 *
 * <li><b>gov.nist.javax.sip.RELIABLE_CONNECTION_KEEP_ALIVE_TIMEOUT</b> Value in seconds which is used as default
 * keepalive timeout
 * (See also http://tools.ietf.org/html/rfc5626#section-4.4.1). Defaults to "infiinity" seconds (i.e. timeout event not
 * delivered).</li>
 *
 * <li><b>gov.nist.javax.sip.SSL_HANDSHAKE_TIMEOUT</b> Value in seconds which is used as default timeout for performing
 * the SSL Handshake
 * This prevents bad clients of connecting without sending any data to block the server</li>
 *
 * <li><b>gov.nist.javax.sip.SSL_RENEGOTIATION_ENABLED = [true|false]</b> Default value is <b>true</b>. Allow or
 * disallow SSL renegotiation to resolve potential DoS problem -
 * <a href="http://web.nvd.nist.gov/view/vuln/detail?vulnId=CVE-2011-1473">reference</a> and
 * <a href="http://www.ietf.org/mail-archive/web/tls/current/msg07553.html">another reference</a>. The safe option is to
 * disable it.</li>
 *
 *
...
public class SipStackImpl extends SIPTransactionStack implements javax.sip.SipStack, SipStackExt {
    ...
    protected SipStackImpl() {
        ...
        try {
            ...
            logger.logWarning(
                    "UTF-8 charset cannot be used this system. This will lead to unpredictable behavior when parsing SIP messages: "
                            + e.getMessage());
        }
    ...
    public SipStackImpl(Properties configurationProperties) throws PeerUnavailableException {
        ...
        try {
            ...
            throw new IllegalArgumentException("Cound not instantiate stack logger " + stackLoggerClassName
        ...
        try {
            ...
            throw new IllegalArgumentException("Cound not instantiate server logger " + stackLoggerClassName
                    + "- check that it is present on the classpath and that there is a no-args constructor defined",
                    ex1);
        } catch (Exception ex) {
            throw new IllegalArgumentException("Cound not instantiate server logger " + stackLoggerClassName
        ...
        // http://java.net/jira/browse/JSIP-415 Prevent Bad Client or Attacker (DoS) to block the TLSMessageProcessor or
        // TLSMessageChannel
        ...
        if (configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE") != null) {
            super.maxTxLifetimeInvite = Integer
                    .parseInt(configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_INVITE"));
        ...
        if (configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE") != null) {
            super.maxTxLifetimeNonInvite = Integer
                    .parseInt(configurationProperties.getProperty("gov.nist.javax.sip.MAX_TX_LIFETIME_NON_INVITE"));
        ...
        super.setDeliverUnsolicitedNotify(Boolean.parseBoolean(
                configurationProperties.getProperty("gov.nist.javax.sip.DELIVER_UNSOLICITED_NOTIFY", "false")));
        ...
        if (threadPoolSize != null) {
            try {
                ...
                if (logger.isLoggingEnabled()) {
                    SipStackImpl.logger.logError("thread pool size - bad value " + ex.getMessage());
                }
            }
        }
        ...
        if (tcpTreadPoolSize != null) {
            try {
                ...
                if (logger.isLoggingEnabled()) {
                    SipStackImpl.logger.logError("TCP post-parse thread pool size - bad value " + tcpTreadPoolSize
                            + " : " + ex.getMessage());
                }
            }
        }
        ...
        if (serverTransactionTableSize != null) {
            try {
                ...
                if (logger.isLoggingEnabled()) {
                    SipStackImpl.logger.logError("transaction table size - bad value " + ex.getMessage());
                }
            }
        } else {
        ...
        if (configurationProperties.containsKey(SECURITY_MANAGER_PROVIDER_KEY)) {
            ...
            try {
                ...
                throw new PeerUnavailableException(
                        "can't find or instantiate SecurityManagerProvider implementation: " + path, e);
            }
        } else {
        ...
        String connTimeout = configurationProperties.getProperty("gov.nist.javax.sip.CONNECTION_TIMEOUT");
        if (connTimeout != null) {
            try {
        ...
        if (stunAddr != null) {
            SipStackImpl.logger.logWarning("Ignoring obsolete property " + "gov.nist.javax.sip.STUN_SERVER");
        }
        ...
        if (interval != null) {
            try {
                ...
                if (logger.isLoggingEnabled()) {
                    logger.logError(
                            "THREAD_AUDIT_INTERVAL_IN_MILLISECS - bad value [" + interval + "] " + ex.getMessage());
                }
            }
        }
        ...
        this.generateTimeStampHeader = Boolean
                .valueOf(configurationProperties.getProperty("gov.nist.javax.sip.AUTO_GENERATE_TIMESTAMP", "false"))
                .booleanValue();
        ...
        String cipherSuitesStr = configurationProperties.getProperty("gov.nist.javax.sip.ENABLED_CIPHER_SUITES");
        ...
        super.setPatchWebSocketHeaders(Boolean.parseBoolean(
                configurationProperties.getProperty("gov.nist.javax.sip.PATCH_SIP_WEBSOCKETS_HEADERS", "true")));

        super.setPatchRport(Boolean
                .parseBoolean(configurationProperties.getProperty("gov.nist.javax.sip.ALWAYS_ADD_RPORT", "false")));

        super.setPatchReceivedRport(Boolean.parseBoolean(
                configurationProperties.getProperty("gov.nist.javax.sip.NEVER_ADD_RECEIVED_RPORT", "false")));
        ...
        super.setConnectionLingerTimer(
                Integer.parseInt(configurationProperties.getProperty("gov.nist.javax.sip.LINGER_TIMER", "8")));
        ...
        try {
            ...
            logger.logError("Bad configuration value for gov.nist.javax.sip.NIO_BLOCKING_MODE=" + nioMode, e);
        }
        ...
        boolean aggressiveCleanup = Boolean.parseBoolean(
                configurationProperties.getProperty("gov.nist.javax.sip.AGGRESSIVE_CLEANUP", Boolean.FALSE.toString()));
        ...
        boolean sslRenegotiationEnabled = Boolean.parseBoolean(
    ...
    public synchronized ListeningPoint createListeningPoint(String address, int port, String transport)
        ...
        if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
            logger.logDebug(
                    "createListeningPoint : address = " + address + " port = " + port + " transport = " + transport);
        }
        ...
        if (lip != null) {
            ...
            try {
                ...
                if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
                    logger.logDebug(
                            "Created Message Processor: " + address + " port = " + port + " transport = " + transport);
                }
                ...
                if (logger.isLoggingEnabled()) {
                    logger.logError(
                            "Invalid argument address = " + address + " port = " + port + " transport = " + transport);
                }
    ...
    public SipProvider createSipProvider(ListeningPoint listeningPoint) throws ObjectInUseException {
        ...
        if (logger.isLoggingEnabled(LogLevels.TRACE_DEBUG)) {
            SipStackImpl.logger.logDebug("createSipProvider: " + listeningPoint);
        }
    ...
    public void addLogAppender(org.apache.log4j.Appender appender) {
        if (SipStackImpl.logger instanceof gov.nist.core.LogWriter) {
            ((gov.nist.core.LogWriter) SipStackImpl.logger).addAppender(appender);
        }
    }
    ...
    public org.apache.log4j.Logger getLogger() {
        if (SipStackImpl.logger instanceof gov.nist.core.LogWriter) {
            return ((gov.nist.core.LogWriter) SipStackImpl.logger).getLogger();
        }
        return null;
    }
